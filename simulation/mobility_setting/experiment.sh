...
while [  $COUNTER -lt $((numRun+1)) ]; do
   python3 wns_v2_mobility.py $numMobileUser $numNetwork $timeStepDuration $maxNumIteration $COUNTER $algorithmIndex $beta $gainScale $maxTimeStepPrevBlock $dir $saveMinimalDetail $networkDataRate
   now=$(date +%s)
   diff=$(($now-$prevTime))
   prevTime=$now
   if [ $diff -ge 60 ]
