...
NUM_PHASES = 3 if setup == 1 else 2
# create a csv file rateOfConvergence.csv to save the rate of convergence
for i in range(NUM_PHASES): # create a directory for each phase
    mkdir(dir + "PHASE_" + str(i + 1))
    chmod(dir + "PHASE_" + str(i + 1), 0o777)
...
# initially 11 users in each the service area; then 9 users join and leave after some time, leaving the same 11 users in the area
numUsersPerPhase = [11, 20, 11] if setup == 1 else [20, 4]
for part in range(NUM_PHASES):
    print("# users in phase", part + 1, " - ", numUsersPerPhase[part])
    for i in range(numUsersPerPhase[part]):
