...
#         self.totalBandwidthCurrentBlock = 0          # keep running total of bandwidth obtained in current block; to be used to get average at end of block
    ...
    def stableHybridBlockExp3_reset(self, env, startTime, endTime):
        ...
        while t <= MAX_NUM_ITERATION:
            ...
            if t >= startTime and t <= endTime:
                if t == startTime:print(colored("user " + str(self.userID) + " is active at t = " + str(t), "white", "on_red"))
                ...
                if maxProbability >= CONVERGED_PROBABILITY and self.blockLengthPerNetwork[convergedNetworkIndex] >= minBlockLengthPeriodicReset\
                    ...
                    if (maxProbability >= CONVERGED_PROBABILITY and self.blockLengthPerNetwork[convergedNetworkIndex] >= minBlockLengthPeriodicReset):
                        ...
                        # print(colored("user " + str(self.userID) + ", PERIODIC reset @t = " + str(t) + "!!!!!" + ", prob: " + str(self.probability), "yellow")); # input()
                        ...
                        #print(colored("user " + str(self.userID) + ", reset DUE TO NETWORK QUALITY DROP @t = " + str(t) + "!!!!!", "magenta")); # input()
                        self.totalNumResetDrop += 1
                        self.resetDropTimeSlotList.append(t)
                    # if DEBUG == -1: print(colored("@ t = " + str(t) + ", user " + str(self.userID) + " will have a periodic reset of block length, block length: " + str(self.blockLengthPerNetwork) + ", num block network selected: " + str(self.numBlockNetworkSelected) + ", probability:" + str(self.probability) + ", block lengths: " + str(self.blockLengthPerNetwork), "red", "on_white"))
                ...
                if t == (endTime + 1):
                    ...
                    print(colored("user " + str(self.userID) + " is not active as from t = " + str(t), "white", "on_red"))
        ...
        # end stableHybridBlockExp3_resetBlockLength_periodic
    ...
    def resetNetworkBlockLength(self, initialBlockLength):
        ...
        # print(">>> user", self.userID," ----- RESET OF BLOCK LENGTH!!!!!")
    ...
    def stableHybridBlockExp3(self, env, startTime, endTime):
        ...
        while t <= MAX_NUM_ITERATION:
            if self.userID == 1 and DEBUG == -1: print(colored("user " + str(self.userID) + ", time slot " + str(t), "red"))
            if t >= startTime and t <= endTime:
    ...
    def selectGreedy(self):
        ...
        if self.numNetHighestMaxAvg == 1:
            ...
            # print("user", self.userID, ", numNetHighestMaxAvg:", self.numNetHighestMaxAvg, ", averagePerNet:", averagePerNet, ", total bandwidth: ", self.totalBandwidthPerNetwork, ", # time selected:", self.numTimeStepNetworkSelected); input()
        # bestNetIndex = averagePerNet.index(maxAvgBandwidth)
...
#         print("user", self.userID, ", gamma = ", gamma, ", estimated gain = ", estimatedGain, ", weight before normalization: ", self.weight)
    ...
    def isCurrentNetworkWorse(self, scaledGain):    # scaled gain does not include switching cost
        triedAllNet = (0 not in self.numBlockNetworkSelected)
        if self.userID == 1 and DEBUG == -1: print('----- in isCurrentNetworkWorse')
...
for i in range(NUM_MOBILE_USERS - numDynamicUsers, NUM_MOBILE_USERS):
    if ALGORITHM == 1: proc=env.process(mobileUserList[i].exp3(env, MAX_NUM_ITERATION//3 + 1, 2 * MAX_NUM_ITERATION//3))                                                            # each mobile user object calls the method exp3
    elif ALGORITHM == 2: proc=env.process(mobileUserList[i].blockExp3(env, MAX_NUM_ITERATION//3 + 1, 2 * MAX_NUM_ITERATION//3))                                                 # each mobile user object calls the method blockExp3
    elif ALGORITHM == 3: proc=env.process(mobileUserList[i].hybridBlockExp3(env, MAX_NUM_ITERATION//3 + 1, 2 * MAX_NUM_ITERATION//3))                                       # each mobile user object calls the method hybridBlockExp3
    elif ALGORITHM == 4: proc=env.process(mobileUserList[i].stableHybridBlockExp3(env, MAX_NUM_ITERATION//3 + 1, 2 * MAX_NUM_ITERATION//3))                             # each mobile user object calls the method epochBlockExp3
    elif ALGORITHM == 5: proc=env.process(mobileUserList[i].stableHybridBlockExp3_reset(env, MAX_NUM_ITERATION//3 + 1, 2 * MAX_NUM_ITERATION//3))      # each mobile user object calls the method epochBlockExp3
    elif ALGORITHM == 6: proc=env.process(mobileUserList[i].greedy(env, MAX_NUM_ITERATION//3 + 1, 2 * MAX_NUM_ITERATION//3))                                                      # each mobile user object calls the method greedy
    elif ALGORITHM == 7: proc=env.process(mobileUserList[i].expWeightedAvgFullInfo(env, MAX_NUM_ITERATION//3 + 1, 2 * MAX_NUM_ITERATION//3))                            # each mobile user object calls the method expWeightedAvgFullInfo
    elif ALGORITHM == 8: # each mobile user object calls the method expWeightedAvgFullInfo
        if i < 2: proc=env.process(mobileUserList[i].centralized(env, 1, MAX_NUM_ITERATION//3 + 1, 2 * MAX_NUM_ITERATION//3))
        elif i >= 2 and i < 6: proc=env.process(mobileUserList[i].centralized(env, 2, MAX_NUM_ITERATION//3 + 1, 2 * MAX_NUM_ITERATION//3))
        else: proc=env.process(mobileUserList[i].centralized(env, 3, MAX_NUM_ITERATION//3 + 1, 2 * MAX_NUM_ITERATION//3))
    elif ALGORITHM == 9: proc=env.process(mobileUserList[i].fixedRandom(env, MAX_NUM_ITERATION//3 + 1, 2 * MAX_NUM_ITERATION//3))                                             # each mobile user object calls the method fixedRandom
