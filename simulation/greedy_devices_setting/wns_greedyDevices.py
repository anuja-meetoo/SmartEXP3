...
class Network(object):
    ...
    def getPerUserBandwidth(self, delay = 0): # default value of newlyJoined = False
        ...
        '''
        global TIME_STEP_DURATION

        # randomNoise = random.random() * self.dataRate/5    # add some noise to bit rate observed
        # randomNoise = 0
        # print("bit rate:", self.dataRate, ", # user:", self.numUser, ", time slot duration:", TIME_STEP_DURATION, ", delay:", delay); input()
...
#         self.totalBandwidthCurrentBlock = 0          # keep running total of bandwidth obtained in current block; to be used to get average at end of block
    ...
    def stableHybridBlockExp3_reset(self, env):
        ...
        print("user ID:", self.userID, "in stableHybridBlockExp3_reset.....", MAX_NUM_ITERATION)

        if self.userID == 1 and DEBUG == -1: print("user", self.userID, ", maxGain:", self.maxGain)
        while t <= MAX_NUM_ITERATION:

            yield env.timeout(10)
            # if self.userID == 1 and DEBUG == -1:
            if self.userID == 1 or self.userID == NUM_MOBILE_USERS: print(colored("user " + str(self.userID) + ", time slot " + str(t) + ", b = " + str(b) + ", gamma = " + str(gamma) + ", MAX:" + str(MAX_NUM_ITERATION), "red"))
            ...
            if self.userID == 1 and DEBUG == -1: print("num consecutive slots:", self.numConsecutiveSlotPreferredNetwork, ", len gain list:", len(self.preferredNetworkGainList), ", rolling avg window size:", gainRollingAvgWindowSize)
            ...
            #if isNashEquilibrium(): MobileUser.saveRateOfConvergence(self, t)  # if NE is reached, save a record in rateOfConvergence.csv file
            ...
            ##### end check if need to switch back #####
            ...
            print("5:", t, "MAX_NUM_ITERATION:", MAX_NUM_ITERATION, "?", t <= MAX_NUM_ITERATION)
        print("t = ", t, "----- user", self.userID, " DONE!")
    ...
    def networkQualityDeclined(self):
        ...
        if self.userID == 1 and DEBUG == -1: print("user ", self.userID, ", changeInGain: ", changeInGain, ", changeInGain < 0:", changeInGain < 0, ", (abs(changeInGain) >= (percentageDeclineForReset * initialGain) / 100): ", (abs(changeInGain) >= (percentageDeclineForReset * initialGain) / 100), ", gain:", self.preferredNetworkGainList, ", gain rolling avg:", gainList)
...
#             self.coinFlip = self.chooseGreedily = self.switchPrevNet = 0
            ...
            if self.resetToPrevNetwork == False:
                ...
                if self.blockLength == self.blockLengthPerNetwork[networkIndex]:
                    if MobileUser.isCurrentNetworkWorse(self, scaledGain) == True:
                        self.resetToPrevNetwork = True
                        self.blockLength = 1
                        if self.userID == 1 and DEBUG == -1: print("user", self.userID, ", current network: " + str(self.currentNetwork), " will switch back!!!"); input()
            else: self.resetToPrevNetwork = False
            ##### check if need to switch back to previous network #####
    ...
    def selectGreedy(self, type="naive", prevNetwork=-1):
        ...
        if self.numNetHighestMaxAvg == 1:
            ...
            if type != "naive" and prevNetwork in bestNetworkIDlist: networkSelected = prevNetwork; self.greedyStaySameNet = 1; self.log.append("STAYING IN PREV NETWORK"); #print("user", self.userID, ", will select prev network", prevNetwork, "as it is one of those with highest average gain ")
    ...
    def selectWirelessNetwork_stableHybridBlock(self, b, maxProbDiff):
        ...
        # if self.totalNumResetPeriodic > 0 and self.networkToExplore != []:
        ...
        # for greedy or random selection
            ...
            if triedAllNet == True and (probNearUniform == True or self.blockLengthPerNetwork[maxProbIndex] <= self.blockLengthForGreedy):
                ...
                if coinFlip == 1:  # use deterministic approach ''' UPDATED THIS '''
                    networkSelected = MobileUser.selectGreedy(self, "smart", prevNetworkSelected)
                    self.log.append("coin flipped... choose network " + str(networkSelected) + " with prob 1...")
                    if self.userID == 1 and DEBUG == -1: print("coin flipped... choose network " + str(networkSelected) + " with prob 1...")
                    self.chooseGreedily = 1
                else:
                    networkSelected = np.random.choice(self.availableNetwork, p=self.probability)
                    self.log.append("coin flipped... but will not use deterministic version...use randomized Exp3")  # ''' UPDATED THIS '''
                    if self.userID == 1 and DEBUG == -1: print("coin flipped... but will not use deterministic version...use randomized Exp3")
                self.coinFlip = 1

            else:
                networkSelected = np.random.choice(self.availableNetwork, p=self.probability)
                if self.userID == 1 and DEBUG == -1: print("use randomized Exp3")
...
#             self.coinFlip = self.chooseGreedily = 0
            ...
            yield env.timeout(10)
            scaledGain = MobileUser.observeGain(self, prevNetworkSelected)                 # (3) observe bandwidth obtained from the wireless network selected, and re-scale it to the range [0, 1]
            # print(">>> @t =", t, ", user", self.userID, " going to call computeEstimatedGain")
            estimatedGain = MobileUser.computeEstimatedGain(self, scaledGain, prevNetworkSelected)                           # (4) compute estimated gain
...
#             self.totalBandwidthCurrentBlock += scaledGain #  keep a running total of the amount of bandwidth (scaled) obtained from the selected network over time steps in the current block
    ...
    def greedy(self, env):
        ...
        print("user ID:", self.userID, "in greedy.....")

        while t <= MAX_NUM_ITERATION: #True:
            yield env.timeout(10)
            if self.userID == 1 or self.userID == NUM_MOBILE_USERS: print(colored("user " + str(self.userID) + ", time slot " + str(t) + ", MAX:" + str(MAX_NUM_ITERATION), "red"))
            networkFileWritten = False
            prevNetworkSelected = self.currentNetwork
    ...
    def expWeightedAvgFullInfo(self, env):
        ...
        while t <= MAX_NUM_ITERATION: #True:
            ...
            if isNashEquilibrium(): MobileUser.saveRateOfConvergence(self, t)  # save rate of convergence to csv file
            elif(t == MAX_NUM_ITERATION): MobileUser.saveRateOfConvergence(self, -1)  # save rate of convergence to csv file
    ...
    ''' ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- '''
    def centralized(self, env, networkAssigned):
        global convergenceFileWritten, networkFileWritten # global variables
        t = 1   # initialize the iteration number
        prevWeight = [0] * len(self.availableNetwork) # not required; only for calling saveUserDetails
...
#         maxDelay = TIME_STEP_DURATION - 0.1
    ...
    def computeEstimatedGain(self, scaledGain, prevNetworkSelected):
        ...
        if self.explorationAfterReset == 1:
        ...
        elif self.switchPrevNet == 1: prob = 1#; print("SWITCH BACK:- user", self.userID, ", net:", self.currentNetwork, ",  current block prob: ", prob) # switch back to previous network; choice was deterministic
        elif self.chooseGreedily == 1:
            if self.greedyStaySameNet == 1:
                prob = 1/2 ;
                if self.userID == 1 and DEBUG == -1: print("GREEDY STAY SAME NETWORK:- user", self.userID, ", current net:", self.currentNetwork, ", current block prob: ", prob)
            ...
            if self.numNetHighestMaxAvg > 1:
                if self.userID == 1 and DEBUG == -1: print("user", self.userID, ", current block prob: ", prob)
        else:   # random choice
...
#         print("user", self.userID, ", gamma = ", gamma, ", estimated gain = ", estimatedGain, ", weight before normalization: ", self.weight)
        ...
        if networkFileWritten == False:
            ...
            for net in networkList:
                ...
                for user in net.userIDlist:
                    if userIDlistCurrentNet == "":
                        userIDlistCurrentNet += str(user)
                    else:
                        userIDlistCurrentNet += "," + str(user)
    ...
    ''' ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- '''
    def saveRateOfConvergence(self, t):
        '''
        ...
        if convergenceFileWritten == False:
            ...
            filename = OUTPUT_DIR+"rateOfConvergence.csv"
            data=[RUN_NUM, t, self.userID]
            for i in range(NUM_NETWORK):
                data.append(networkList[i].numUser)
...
'''
proc=env.process(mobileUserList[0].greedy(env))
for i in range(1, NUM_MOBILE_USERS):
    proc = env.process(mobileUserList[i].stableHybridBlockExp3_reset(env))

##### scenario 2 - 10 greedy and 10 smart EXP3

for i in range(NUM_MOBILE_USERS//2):
    proc = env.process(mobileUserList[i].greedy(env))
for i in range(NUM_MOBILE_USERS // 2, NUM_MOBILE_USERS):
    proc = env.process(mobileUserList[i].stableHybridBlockExp3_reset(env))
'''
# ##### scenario 3 - 1 smart EXP3 and  19 greedy
proc=env.process(mobileUserList[0].stableHybridBlockExp3_reset(env))
for i in range(1, NUM_MOBILE_USERS):
    proc = env.process(mobileUserList[i].greedy(env))
