...
def timeoutHandler(signum, frame):
    ...
    global blockLength, maxGain, algorithmName, lock
...
def disconnect(previousNetwork):
    ...
    try:
        ...
        # process = subprocess.Popen("sudo nmcli dev disconnect wlan0", shell=True, stdout=subprocess.PIPE)
        # status = process.wait()
...
def smartEXP3():
    ...
    if timeSlot == 1: # first time slot
        ...
        # print(colored("setting selection type to 2", "yellow"))
...
def wns():
    ...
    global startSelectionTime, downloadProcess, BUFFER_SIZE, x, startWnsTime, lock, delayCarriedToNextTimeSlot
    global delayPerTimeSlot, numByteDownloadedPerTimeSlot, numByteDownloadedDuringSelectionPerTimeSlot, logPerTimeSlot
    ...
    try:
        ...
        if DEBUG >= 1: print("@t = ", timeSlot, ", in wns..... len(logPerTimeSlot): ", len(logPerTimeSlot), ", lapsedTime:", time() - startSelectionTime)
            ...
            if isConnected(currentNetwork):
                ...
                if downloadProcess.is_alive():
                    if (numByteDownloadedPerTimeSlot[-1] != 0) or (numByteDownloadedPerTimeSlot[-1] == 0 and delayCarriedToNextTimeSlot[0] != 0):
                        ...
                        # print("setting delay to delayCarriedToNextTimeSlot " + str(delayCarriedToNextTimeSlot.value)); delayPerTimeSlot.append(delayCarriedToNextTimeSlot.value); delayCarriedToNextTimeSlot.value = 0
...
# numByteRecvPerTimeSlot = [] # TO BE REMOVED
...
out = csv.writer(outfile, delimiter=',', quoting=csv.QUOTE_NONNUMERIC)
if algorithmName == "EXP3":
    out.writerow(["time slot", "timestamp", "date time", "gamma", "max gain", "weight", "probability", "network selected", "delay", "#bytes", "#bytes w/o delay", "bit rate (Mbps)", "selection duration", "connect and download duration", "#bytes downloaded during selection by previous network", "log"])
elif algorithmName == "smartEXP3":
...
while timeSlot < (numIteration + 1):
    try:
        timeSlot += 1;
        if DEBUG >= 1:
            print(colored("going to increment timeslot to " + str(timeSlot), "green", "on_white"));
            print(colored("incremented timeslot to " + str(timeSlot) + "@ " + str(time()), "green", "on_white"))  # increment time step index

        # register the signal function handler
        signal.signal(signal.SIGALRM, timeoutHandler)     # for function timeout

        # define a timeout for your function
        if DEBUG >= 1: print("timeSlot: ", timeSlot, "downloadProcess.is_alive()?", downloadProcess.is_alive())
        if timeSlot == 1:
            ...
            print(colored("timeslot = " + str(timeSlot) + ", previous time slot:- duration = " + str(startSelectionTime - prevStartSelectionTime) + ", delay = " + str(delayPerTimeSlot[-1])  + ", #bytes recv = " + str(numByteDownloadedPerTimeSlot[-1]) + ", bit rate: " + str(bitRatePerTimeSlot[-1]), "red", "on_white"))
            if startSelectionTime - prevStartSelectionTime > (timeSlotDuration + 1): countExceedTimeSlotDuration += 1
            if numByteDownloadedPerTimeSlot[-1] == 0: countZeroByte += 1; timeSlotZeroByte.append(timeSlot - 1)
        ...
        if DEBUG >= 1: print(colored("time slot = " + str(timeSlot) + ", x = " + str(x), "yellow"))
        signal.alarm(int(x))
